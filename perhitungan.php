...
while ($pen = mysqli_fetch_array($querypen)) {
    ...
    while ($krt = mysqli_fetch_array($querykriteria)) {
        ...
        $querynilai = mysqli_query($con, "SELECT nilai_kriteria FROM tbl_penilaian where nisn='$id_pen' AND id_kriteria='$id_krt' AND tgl_daftar='$tgl_daftar'");
        ...
        $queryNormalisasi = mysqli_query($con, "UPDATE tbl_penilaian SET nilai_normalisasi = '$nilai_normalisasi_format' where nisn='$id_pen' AND id_kriteria='$id_krt' AND periode ='$priode' AND tgl_daftar='$tgl_daftar'");
    }
}


$querypen1 = mysqli_query($con, "SELECT * FROM tbl_peserta");
while ($pen1 = mysqli_fetch_array($querypen1)) {
    ...
    while ($krt1 = mysqli_fetch_array($querykriteria1)) {
        ...
        // echo $bobot;
        //
        $queryhasil = mysqli_query($con, "SELECT nilai_normalisasi FROM tbl_penilaian where nisn='$id_pen1' AND id_kriteria='$id_krt1' AND periode='$priode' AND tgl_daftar='$tgl_daftar'");
    ...
    $simpan = mysqli_query($con, "UPDATE tbl_penilaian SET nilai_v = $hasil where nisn='$id_pen1' AND periode ='$priode' AND tgl_daftar='$tgl_daftar'");
    // echo $hasil . "<br>";
}
echo "<script>window.location='detailPenilaian.php?tgl=$tgl_daftar&priode =$priode';</script>";
